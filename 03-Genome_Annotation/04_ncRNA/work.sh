...
ln -s /path/Rfam_anno-snRNA.txt

tRNA Annotation using tRNAscan
source /path/conda.sh
conda activate /path/tRNAscan
conda info --envs
...
/path/cmscan --cpu 10 -Z 5271121.81 --cut_ga --rfam --nohmmonly --fmt 2 --tblout CanA_ncRNA.tblout -o CanA_ncRNA.result --clanin $R/Rfam.clanin $R/Rfam.cm ../../Celastrus_angulatus_A_genome.fasta
...
/path/bin/barrnap --kingdom euk --threads 10 --outseq CanA_rRNA.fa  CanA_genome.tmp.fasta > CanA_rRNA.gff3
