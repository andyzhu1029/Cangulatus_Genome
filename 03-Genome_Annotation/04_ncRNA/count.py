...
class ncRNAgff:
    def __init__(self,path:str):        
        ...
        #     self.data[name]=[]
        for i in range(len(out_list[3].split('\t'))):
           # out_list.append([])
            self.data.append([])
        for row in range(3,len(out_list)):
    ...
    def data_analysis(self):
        begin_data = np.array(self.data[2])
        end_data = np.array(self.data[3])
