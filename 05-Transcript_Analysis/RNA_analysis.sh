...
#16th April 2023
#===========================================================================================================================
export PATH=~/path/bin:$PATH #python3
export PATH=/path/bin:$PATH #cutadpter v2.10

echo =========start at : `date` ===========================================================
for id in $(cat sample_list)
do
trim_galore -o ./Clean_seq --paired ./$id/${id}_1.fq.gz ./$id/${id}_2.fq.gz
done
...
for species in A B
do
hisat2-build Can_${species}_genome.fasta ./${species}_hisat2_index/Can${species}_index 
done
...
do
	for id in $(cat sample_list)
	do 
	hisat2 --dta -p 20 -x ./${species}_hisat2_index/Can${species}_index \
...
do
	for id in $(cat sample_list)
	do
	/path/samtools-1.9/samtools sort -@ 20 \
...
do
	for id in $(cat sample_list)
	do
	stringtie -p 20 -e -B -G ./Cangulatus_${species}_chr.gff3 -l $id \
